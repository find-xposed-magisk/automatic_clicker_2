...
from WindowControl.截图模块 import ScreenCapture
from 数据库操作 import DatabaseOperation
from WindowControl.窗口状态 import install_window_state
from Window.图像选择_ui import Ui_ImageSelect
from Window.导航窗口_ui import Ui_navigation
from 网页操作 import WebOption
from WindowControl.设置窗口 import Setting
from WindowControl.选择窗体 import Variable_selection_win, ShortcutTable
...
class ImageSelection(QDialog, Ui_ImageSelect):
    ...
    def load_images_name_to_listView(self):
        """加载图片名称到listView"""
        images_name_list = self.db.get_all_png_images_from_resource_folders()
        # 创建模型并绑定到listView
    ...
    def preview_image(self):
        ...
        # 获取图片路径
        image_path = self.db.matched_complete_path_from_resource_folders(image_name)
        # 加载图片
        if image_path != '':
            # 将图像转换为QImage对象
...
class Na(QWidget, Ui_navigation):
    ...
    def __init__(self, main_window_=None):
        ...
        self.setupUi(self)
        self.db = getattr(main_window_, "db", None) or DatabaseOperation()
        # 去除最大化最小化按钮
        self.setWindowFlags(Qt.WindowType.WindowCloseButtonHint)
        self.setWindowModality(Qt.WindowModality.ApplicationModal)
        install_window_state(
            self, self.db, self.windowTitle(), enable_maximize_button=False
        )
    ...
    def image_click_function(self, type_):
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_8.addItems(self.db.extract_resource_folder_path())
            self.find_controls("图像", "图像点击")
            self.show_image_to_label("图像点击")
            # 设置初始识别精度
    ...
    def multiple_images_click_function(self, type_):
        ...
        def quick_screenshot_and_add_image():
            ...
            def get_the_latest_saved_image():
                ...
                latest_mod_time = 0
                for folder_path in self.db.extract_resource_folder_path():
        ...
        def delect_selected_image():
            ...
            if selected_image:
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    # 获取图像完整路径
                    image_path = self.db.matched_complete_path_from_resource_folders(selected_image)
                    # 刷新listView
    ...
    def image_waiting_function(self, type_):
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_17.addItems(self.db.extract_resource_folder_path())
    ...
    def information_entry_function(self, type_):
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_12.addItems(
                self.db.extract_excel_from_resource_folders()
            )  # 加载全局参数中的excel文件路径
            self.comboBox_13.clear()
            self.comboBox_14.clear()
            self.comboBox_14.addItems(self.db.extract_resource_folder_path())
            self.find_controls("图像", "信息录入")
    ...
    def web_entry_function(self, type_):
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_20.addItems(self.db.extract_excel_from_resource_folders())
    ...
    def save_form_function(self, type_):
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_29.addItems(self.db.extract_excel_from_resource_folders())
        elif type_ == "还原参数":
            put_parameters(self.image_path, self.parameter_1)
    ...
    def full_screen_capture_function(self, type_):
        ...
        def get_parameters():
            ...
            image_path_ = self.db.portable_resource_path(
        ...
        def put_parameters(image_path_, parameter_dic_):
            """将参数还原到控件中"""
            image_path_ = self.db.resolve_resource_path(image_path_)
            # 还原截图类型
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_31.addItems(self.db.extract_resource_folder_path())
            show_region()
            show_save_path()
        elif type_ == "还原参数":
            put_parameters(self.image_path, self.parameter_1)
    ...
    def input_cell_function(self, type_):
        ...
        if type_ == "按钮功能":
            ...
            self.comboBox_57.addItems(self.db.extract_excel_from_resource_folders())
            self.find_controls("excel", "写入单元格")
        elif type_ == "还原参数":
            put_parameters(self.image_path, self.parameter_1)
    ...
    def ocr_recognition_function(self, type_):
        ...
        def test():
            ...
            try:
                ...
                # 测试用例
                client_info = self.db.get_ocr_info()
